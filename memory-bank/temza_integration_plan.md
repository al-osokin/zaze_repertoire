...
2. **Нормализация актёров** — делить слипшиеся фамилии/имена при импорте (regex `([а-яё])([А-ЯЁ])` → `$1, $2`), чтобы в `temza_cast_resolved` сразу хранились корректные списки.  
...
4. **Уведомления о заменах** — после нового импорта сравнивать составы с уже подтверждёнными карточками и выводить предупреждения («10.12.25 Синяя птица: Старец Время — Сазонов → Удалов»).  
