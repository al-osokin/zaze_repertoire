...
*   Для восстановления испорченных составов используем скрипт `scripts/rebuild_cast_from_vk_text.php`: он сопоставляет роли из шаблона и исполнителей из сохранённого `vk_post_text` и пересоздаёт `performance_roles_artists`. При отсутствии карточки спектакль очищаем полностью и заполняем заново вручную.
*   Ошибочные артисты удаляются или переназначаются через одноразовые cleanup-скрипты, но перед коммитом все временные SQL/CSV-файлы стираем, чтобы в репозитории оставалось только рабочее состояние.
*   В админке добавлено общее навигационное меню (Афиша/Парсинг/Спектакли + шестерёнка VK) и убраны разрозненные кнопки «Главная/Парсинг» на отдельных страницах.
