# Система управления репертуаром театра 🎭

**Полностью готовая к использованию система** для создания репертуарных афиш театра с автоматическим преобразованием текста в вики-разметку.

## ✅ Возможности

- **Преобразование черновика афиши в вики-разметку**
- **Управление базой спектаклей** с сокращениями и полными названиями (56 спектаклей в базе)
- **Шаблоны страниц** для спектаклей с ролями
- **История созданных афиш** с возможностью просмотра
- **Защищённый доступ** по логину/паролю
- **Копирование результатов** в буфер обмена одним кликом
- **Гибкое форматирование времени** (HH и HHMM форматы: 15:00, 15:30)
- **Автоматическая обработка продолжения дат** (строки с табуляцией)
- **Умная генерация ссылок** для повторяющихся спектаклей
- **Поддержка точного времени** (15:30, 19:45 и т.д.)
- **Специальная обработка абонементов** с внешними ссылками на билеты
- **Специальные отметки** для премьер и других обозначений ("ПРЕМЬЕРА!", "БЕНЕФИС" и т.д.)
- **Обработка ошибок** с понятными сообщениями при неизвестных спектаклях
- **Обработка дубликатов** спектаклей с пользовательскими подсказками
- **Совместимость с Windows** (корректная обработка CRLF окончаний строк)
- **Гибкие регулярные выражения** (работают с пробелами и табуляциями)
- **Современный интерфейс** с иконками, toast уведомлениями и подсказками

## 🚀 Последние улучшения

### Версия 1.0 - Полностью готова к использованию

**✅ Исправлены критические проблемы:**
- **Windows CRLF совместимость** - корректная обработка файлов созданных в Windows
- **Гибкие регулярные выражения** - работают с любыми комбинациями пробелов и табуляций
- **Полная обработка продолжения дат** - все спектакли в один день распознаются правильно

**✅ Улучшения пользовательского интерфейса:**
- **Обработка ошибок** - понятные сообщения при неизвестных спектаклях
- **Валидация данных** - проверка перед сохранением в историю
- **Стилизация ошибок** - красный фон для сообщений об ошибках

**✅ База данных:**
- **56 спектаклей** полностью настроены и готовы к работе
- **Шаблоны страниц** для генерации wiki-страниц с ролями
- **Полный дамп БД** в `schema.sql` для быстрого развертывания

**✅ Современный пользовательский интерфейс:**
- **Иконки вместо текста** на кнопках управления спектаклями
- **Toast уведомления** - современные всплывающие сообщения
- **Подсказки к полям** - помощь при заполнении форм
- **Кнопка "Очистить форму"** - быстрый сброс всех полей
- **Автоматическая инициализация** поля full_name с вики-разметкой
- **Обработка дубликатов** спектаклей с понятными сообщениями

**✅ Улучшения дизайна и стилизации:**
- **Tailwind CSS интеграция** - современная система стилизации
- **Поддержка тёмной темы** - автоматическое переключение между светлой и тёмной темами
- **Современная цветовая палитра** - профессиональные цвета для театральной тематики
- **Адаптивный дизайн** - корректное отображение на всех устройствах

**✅ Технические улучшения:**
- **Отладочная система** - подробные логи для диагностики проблем
- **Безопасность** - комплексная защита с помощью `.htaccess`
- **Производительность** - оптимизированные запросы к БД
- **Совместимость** - работает на shared hosting и VPS
- **AJAX обработка** - копирование шаблонов без перезагрузки страницы

### Текущий статус: 🟢 ПРОДАКШЕН ГОТОВ

Система полностью протестирована и готова к регулярному использованию для создания театральных афиш.

## 🎯 Быстрый старт

**База данных содержит 56 полностью настроенных спектаклей:**
- многие спектакли театра "Зазеркалье"

**Все функции протестированы и работают:**
- ✅ Преобразование черновиков в вики-разметку
- ✅ Обработка продолжения дат (табуляции)
- ✅ Гибкое форматирование времени
- ✅ Управление спектаклями с иконками
- ✅ История афиш с вкладками
- ✅ Шаблоны страниц с копированием
- ✅ Обработка ошибок и дубликатов
- ✅ Копирование в буфер обмена
- ✅ Toast уведомления
- ✅ Подсказки к полям формы
- ✅ Автоматическая инициализация полей

## Установка

### 1. Настройка базы данных

1. Создайте базу данных MariaDB/MySQL
2. Выполните SQL-скрипт из файла `schema.sql`
3. Скопируйте `config.php.orig` в `config.php` и обновите настройки подключения:
   ```bash
   cp config.php.orig config.php
   ```
4. Откройте `config.php` и замените плейсхолдеры на реальные настройки вашей базы данных

### 2. Настройка сервера

1. Скопируйте файлы на веб-сервер с поддержкой PHP
2. Убедитесь, что PHP имеет расширение PDO для MySQL
3. Настройте права доступа к файлам (рекомендуется 755 для директорий, 644 для файлов)
4. **Если на сервере установлен WordPress**: Замените содержимое корневого `.htaccess` на содержимое файла `.htaccess-base`

#### Настройка Tailwind CSS (опционально)

Система работает без Tailwind CSS, используя традиционные стили из `css/main.css`. Для использования современных стилей из `app/globals.css`:

**Быстрый старт с CDN:**
```html
<!-- Добавьте в <head> ваших PHP файлов -->
<link href="https://cdn.tailwindcss.com" rel="stylesheet">
<link rel="stylesheet" href="app/globals.css">
```

**Полная установка (рекомендуется для продакшена):**
```bash
# Установите Node.js и npm на сервере
npm install -D tailwindcss

# Скомпилируйте стили
npx tailwindcss -i ./app/globals.css -o ./css/tailwind.css --watch
```

**Примечание:** Tailwind CSS не обязателен для работы системы. Все функции доступны с текущими стилями.

### 3. Изменение логина и пароля (опционально)

Если вы хотите изменить стандартные учетные данные:

1. Откройте файл `test/generate_password.php`
2. Измените переменную `$password` на желаемый пароль
3. Запустите скрипт в браузере или через PHP CLI
4. Скопируйте сгенерированный хэш
5. Обновите строку в `schema.sql`:
   ```sql
   INSERT INTO users (username, password_hash) VALUES
   ('ваш_логин', 'скопированный_хэш');
   ```

### 4. Диагностика (при проблемах)

#### Диагностика ошибок 500

Используйте специальные файлы для диагностики:

**test/test.php** - базовый тест PHP и сервера
```
https://new.a-v-o.ru/test/test.php
```
Показывает:
- Версию PHP
- Загруженные расширения
- Информацию о сервере

**test/db_test.php** - проверка подключения к базе данных
```
https://new.a-v-o.ru/test/db_test.php
```
Показывает:
- Статус подключения к БД
- Список таблиц
- Количество пользователей

**test/error_test.php** - подробная диагностика PHP
```
https://new.a-v-o.ru/test/error_test.php
```
Показывает:
- Все PHP ошибки
- Статус загрузки файлов
- Результаты функций

**test/minimal_test.php** - минимальный тест компонентов
```
https://new.a-v-o.ru/test/minimal_test.php
```
Показывает:
- PHP работает
- Сессии работают
- config.php загружается
- БД подключается

#### Решение проблем с .htaccess

Если `.htaccess` вызывает ошибки сервера, используйте альтернативные версии из папки `test/`:

**Вариант 1: Рабочий .htaccess (рекомендуется)**
- Скопируйте `test/.htaccess-working` в корень и переименуйте в `.htaccess`
- Без HTTPS перенаправления, только базовая защита

**Вариант 2: Минимальный .htaccess**
- Скопируйте `test/.htaccess-minimal` в корень и переименуйте в `.htaccess`
- Содержит только базовую защиту файлов

**Вариант 3: Безопасный .htaccess**
- Скопируйте `test/.htaccess-safe` в корень и переименуйте в `.htaccess`
- Без потенциально проблемных директив

**Вариант 4: Пошаговое тестирование**
- Скопируйте `test/.htaccess-debug` в корень и переименуйте в `.htaccess`
- Добавляйте правила по одному для выявления проблемного

**Распространенные проблемы:**
- `php_flag engine on` - запрещено на shared hosting
- `ServerSignature Off` - может быть запрещено
- `ServerTokens Prod` - может быть запрещено
- Конфликт с глобальными настройками Apache

### 5. Первый запуск

1. Откройте в браузере страницу входа в систему
2. Войдите с учетными данными, которые вы настроили в базе данных
3. Перейдите в раздел "Управление спектаклями" для настройки базы

**Примечание:** Система использует безопасную аутентификацию через базу данных. Для изменения учетных данных используйте файл `test/generate_password.php` и обновите `schema.sql`.

## Использование

### Создание афиши

1. Вставьте текст черновика в поле на главной странице
2. Нажмите "Преобразовать"
3. Просмотрите результат
4. При необходимости сохраните в историю

### Формат черновика

```
месяц год
дата время сокращение_спектакля код_мероприятия
дата время сокращение_спектакля код_мероприятия
	время сокращение_спектакля код_мероприятия  # продолжение предыдущей даты
...
```

**Поддерживаемые форматы времени:**
- `HH` - часы (15 → 15:00)
- `HHMM` - часы и минуты (1530 → 15:30)

**Продолжение дат:**
- Строки с табуляцией в начале наследуют дату от предыдущей строки
- Позволяет указывать несколько спектаклей в один день

Пример:
```
сентябрь 2025
18 19 Тоска 1603
19 19 Летучка 1602
20 12 Любимая 1613
	15 СП 1604        # 20 сентября, 15:00
	19 СП 1605        # 20 сентября, 19:00
21 12 Теремок 1611
	1530 Теремок 1610 # 21 сентября, 15:30
```

### Управление спектаклями

В разделе "Управление спектаклями" можно:
- Добавлять/редактировать спектакли с современным интерфейсом
- Настраивать соответствия сокращений и полных названий
- Создавать и редактировать шаблоны страниц с ролями
- Копировать шаблоны одним кликом в буфер обмена
- Использовать иконки для быстрой навигации

#### Интерфейс управления спектаклями

**Таблица спектаклей:**
- ✏️ **Редактировать** - редактирование параметров спектакля
- 👥 **Состав** - управление шаблоном страницы с ролями
- 📋 **Копировать** - копирование шаблона в буфер обмена
- 🗑️ **Удалить** - удаление спектакля из базы

**Форма добавления спектакля:**
- Автоматическая инициализация поля "Полное название" с вики-разметкой `[[|]]`
- Подсказки к каждому полю для правильного заполнения
- Кнопка "Очистить форму" для быстрого сброса
- Обработка дубликатов с понятными сообщениями об ошибках

**Toast уведомления:**
- Зеленые уведомления для успешных операций
- Красные уведомления для ошибок
- Синие информационные сообщения
- Автоматическое исчезновение через 3 секунды

### Шаблоны страниц

Для каждого спектакля можно создать шаблон страницы с ролями. Используйте плейсхолдер `{CODE}` для автоматической подстановки кода мероприятия.

Пример шаблона:
```
==В ролях:==
''СОСТАВ УТОЧНЯЕТСЯ''

'''[https://www.zazerkal.spb.ru/tickets/{CODE}.htm|КУПИТЬ БИЛЕТ]'''
```

## Структура файлов

### Основные файлы (обязательные для работы системы)

- `config.php` - настройки подключения к БД (создайте из `config.php.orig`)
- `config.php.orig` - шаблон конфигурации для GitHub
- `db.php` - функции работы с базой данных
- `parser.php` - логика преобразования текста
- `index.php` - главная страница
- `admin.php` - управление спектаклями
- `history.php` - история афиш
- `login.php` - страница входа
- `get_template.php` - AJAX обработчик для копирования шаблонов
- `schema.sql` - структура базы данных
- `.htaccess` - основные правила безопасности
- `.gitignore` - исключения для Git
- `README.md` - документация

### Папка app/ (стили и дизайн)

- `app/globals.css` - глобальные стили с Tailwind CSS и поддержкой тёмной темы

**Примечание по Tailwind CSS:**
Файл `app/globals.css` содержит современные стили с Tailwind CSS v4, но для его использования требуется дополнительная настройка:

#### Вариант 1: Использование CDN (рекомендуется для быстрого старта)
```html
<link href="https://cdn.tailwindcss.com" rel="stylesheet">
```

#### Вариант 2: Локальная установка (для продакшена)
```bash
npm install -D tailwindcss
npx tailwindcss -i ./app/globals.css -o ./css/tailwind.css --watch
```

#### Вариант 3: Использование готовых классов
Подключите `app/globals.css` после установки Tailwind CSS на сервере.

**Текущий статус:** Файл `globals.css` создан, но не подключен к проекту. Используется `css/main.css` с традиционными стилями.

### Папка test/ (диагностические и тестовые файлы)

#### Диагностические файлы
- `test/test.php` - базовый тест PHP и сервера
- `test/db_test.php` - тест подключения к базе данных
- `test/error_test.php` - подробная диагностика ошибок PHP
- `test/session_test.php` - тест работы сессий
- `test/minimal_test.php` - минимальный тест PHP функций

#### Файлы для отладки входа
- `test/login_test.php` - полный тест процесса входа
- `test/login_debug.php` - отладка входа без редиректа
- `test/login_fixed.php` - версия с JavaScript редиректом
- `test/login_simple.php` - простейшая версия для тестирования
- `test/login_working.php` - рабочая версия входа
- `test/password_check.php` - проверка паролей

#### Служебные файлы
- `test/generate_password.php` - генератор хэшей паролей

#### Альтернативные .htaccess файлы
- `test/.htaccess-working` - рабочая версия без HTTPS перенаправления
- `test/.htaccess-safe` - безопасная версия без потенциально проблемных директив
- `test/.htaccess-minimal` - минимальная версия только с базовой защитой
- `test/.htaccess-debug` - отладочная версия для пошагового тестирования
- `test/.htaccess-base` - для корневой папки WordPress
- `test/.htaccess-original` - оригинальная версия .htaccess

## Безопасность

### Файл .htaccess

Файл `.htaccess` содержит комплексные правила безопасности для Apache:

- **Защита конфиденциальных файлов**: Блокирует прямой доступ к `config.php`, `db.php`, `parser.php`, `schema.sql`
- **Запрет просмотра директорий**: Предотвращает листинг содержимого папок
- **Безопасные заголовки**: Устанавливает заголовки защиты от XSS, кликджекинга и других атак
- **Защита от SQL-инъекций**: Блокирует подозрительные запросы через URL
- **Сжатие контента**: Оптимизирует передачу данных
- **Кэширование**: Ускоряет загрузку статических ресурсов

### Дополнительные меры безопасности

- **HTTPS включен**: Все соединения автоматически перенаправляются на HTTPS
- **Защита от хотлинкинга**: Настроена для домена new.a-v-o.ru
- Используйте сложные пароли для пользователей
- Регулярно обновляйте пароль администратора
- Ограничьте доступ к директории с файлами
- В продакшене отключите отображение ошибок PHP
- Регулярно обновляйте пароль в базе данных (используйте `password_hash()` для генерации)

## Поддержка

При возникновении проблем проверьте:
1. Настройки подключения к БД в `config.php`
2. Права доступа к файлам
3. Версию PHP (рекомендуется 7.4+)
4. Наличие расширения PDO для MySQL

## Разработка

Для внесения изменений:
1. Модифицируйте соответствующие PHP-файлы
2. Обновите структуру БД при необходимости
3. Протестируйте изменения
4. Обновите документацию
